...
const AuthController = {
  ...
  async register(req, res) {
    try {
     ...
     console.log("🔑 Hash gerado para a senha antes de salvar no MongoDB:", hashedPassword);
  ...
  async login(req, res) {
    try {
      ...
      console.log("🔍 Usuário encontrado:", {
        email: user.email,
        role: user.role,
        hashSalvo: user.password, // Agora mostramos o hash real
      });

      // 🔑 Comparação da senha digitada com a armazenada
      const isValid = await bcrypt.compare(password, user.password);
      console.log("🔑 Comparação de senha:", isValid ? "✅ Senha correta" : "❌ Senha incorreta");
